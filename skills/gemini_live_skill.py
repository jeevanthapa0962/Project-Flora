    ...
    def start_live_vision(self, **kwargs):
        ...
        try:
            ...
            if self.pause_event:
                ...
                print("[GeminiLiveSkill] Paused FLORA main loop.")

            # Launch process
            # use sys.executable to ensure we use the same python interpreter (venv)
            process = subprocess.Popen([sys.executable, script_path])
            
            # Spawn a thread to wait for the process to finish and then resume FLORA
            def monitor_process(proc, pause_evt):
                proc.wait()
                if pause_evt:
                    pause_evt.clear()
                    print("[GeminiLiveSkill] Resumed FLORA main loop.")
